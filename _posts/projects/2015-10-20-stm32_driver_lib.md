---
layout: default
title: STM32F10x系列的外设C++驱动库
description: 为了让在STM32上的开发更加便捷、快速，使用C++进行对片上外设和片外模块进行封装，隐藏了对寄存器的操作，将嵌入式的开发也对象化，简化使用的同时也提升了可读性。让程序可以多次重复利用的同时降低了STM32上手难度和开发难度的同时缩短了开发周期。
categories: projects
---
